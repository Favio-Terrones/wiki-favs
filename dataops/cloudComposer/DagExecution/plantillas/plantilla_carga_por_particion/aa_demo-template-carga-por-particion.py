...
# -- Constantes:
CARPETA_MODELO_DATOS = 'demo_carga_por_particion'  # i.e.: slv_modelo_venta_odoo_mk_dex, gld_cliente_sidex
...
with DAG(dag_id=parameter_dag_id, schedule_interval=None, default_args=default_dag_args) as dag:
    ...
    for month in month_list:
        ...
        task_dict[f"script_01-{month}"] >> task_dict[f"script_02-{month}"] >> task_dict[f"script_03-{month}"] >> task_dict[f"script_04_final-{month}"]
        [all_success, all_failed] << task_dict[f"script_04_final-{month}"]
